#!/usr/bin/env python
'''
THis Fork of Wget, the Download utility as an easy way to get file from the net
 
  python -m fget <URL>
  python fget.py <URL>

Wget Downloads: http://pypi.python.org/pypi/wget/
Wget Development: http://bitbucket.org/techtonik/python-wget/

fget.py or wget.py is not option compatible with Unix wget utility,
...
def bar_thermometer(current, total, width=80):
    ...
    return '|' + '\033[0;32;42m \033[0m' * shaded_dots + ' ' * (avail_dots - shaded_dots) + '|\033[1;33;40m ⬇\033[0m'
...
def bar_adaptive(current, total, width=80):
    ...
    for field in selected:
      ...
      if field == 'percent':
        ...
        output += (' %s \033[1;30;40m/\033[0m %s' % (current, total)).rjust(min_width['size'])
...
def detect_filename(url=None, out=None, headers=None, default='download.fget'):
    '''Return filename for saving file. If no filename is detected from output
    argument, url or headers, return default (download.fget)
    '''
...
[ ] write files with unicode characters
    https://bitbucket.org/techtonik/python-fget/issues/7/filename-issue
  [x] Python 2, Windows
  [x] Python 3, Windows
  [ ] Linux
[ ] add automatic tests
